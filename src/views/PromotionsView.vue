<template>
  <div class="promotions-page">
    <div class="aurora-background"><div class="aurora-shape shape1"></div><div class="aurora-shape shape2"></div></div>
    <main class="promo-content">
      <section class="promo-header">
        <div class="container text-center">
          ...
          <p class="main-description">Tu lealtad tiene recompensa. Inicia sesión o regístrate para ganar premios exclusivos.</p>
        </div>
      </section>
      <PromotionalRoulette 
        :user="currentUser"
    ...
    <LoginModal :show="showLoginModal" @close="showLoginModal = false" @openRegister="openRegisterModal" @loggedIn="onLoggedIn"/>
    <RegisterModal :show="showRegisterModal" @close="showRegisterModal = false" @openLogin="openLoginModal" @registered="onRegistered"/>
    <AdminPanel :show="showAdminPanel" @close="showAdminPanel = false"/>
  </div>
</template>
...
// Sincronización al cargar el componente
onMounted(() => {
  // 1. Busca la información del usuario en el localStorage
  const userInfo = localStorage.getItem('userInfo');
  if (userInfo) {
    // 2. Si la encuentra, actualiza la variable reactiva 'currentUser'
    currentUser.value = JSON.parse(userInfo);
  }
  
  // 3. Carga los ajustes de la ruleta
  fetchSettings();
});
...
// Cuando el 'LoginModal' emite 'loggedIn'
...
// Cuando el 'RegisterModal' emite 'registered'
const onRegistered = () => {
  Swal.fire({
...
// Cuando el 'PromotionalRoulette' emite 'logout'
const handleLogout = () => {
    localStorage.removeItem('userInfo');
    currentUser.value = null; // Actualiza el estado reactivo
    Swal.fire({
      icon: 'info',
