    ...
    <div class="container">
      <div class="roulette-layout-wrapper">
        <div ref="rouletteContainerRef" class="roulette-container animate__animated animate__zoomIn">
          <LuckyWheel ref="myLucky" :width="wheelSize" :height="wheelSize" :prizes="prizes" :blocks="blocks" :buttons="buttons" :default-config="defaultConfig" :default-style="defaultStyle" @end="endCallback"/>
        </div>
        ...
        <div class="panel-container animate__animated animate__fadeInRight">
          <div class="panel-header">
            <img src="/images/logo-light.png" alt="Logo de la empresa" class="panel-logo">
          </div>
          <div class="panel-content">
            <div v-if="!user" class="content-body">
              ...
              <button @click="$emit('requestLogin')" class="cta-button-form main-action"><i class="fas fa-sign-in-alt me-2"></i> Iniciar Sesión o Registrarse</button>
            </div>
            <div v-else class="content-body">
              ...
              <div class="prize-display-area">
                <div v-if="!lastPrize" class="prize-display-placeholder"><i class="fas fa-gift"></i><span>Gira para descubrir tu premio</span></div>
                <div v-else class="last-prize-details animate__animated animate__pulse"><p class="last-prize-title">¡Has ganado!</p><h4 class="prize-name">{{ lastPrize.name }}</h4><p class="prize-coupon-label">Usa este cupón al cotizar:</p><p class="prize-coupon">{{ lastPrize.coupon }}</p></div>
              </div>
            </div>
          </div>
...
<script setup>
import { ref, computed, onMounted, onUnmounted, inject, watch } from 'vue'; // Añadimos 'watch'
import { LuckyWheel } from '@lucky-canvas/vue';
import Swal from 'sweetalert2';
import apiClient from '@/api/axios';

// --- Props, Emits e Inyección de Estado ---
...
// ================== DEBUGGING LOG #2 ==================
// 'watch' observará la variable 'user' que viene del padre (vía inject)
// y nos dirá qué contiene cada vez que cambia.
watch(user, (newValue) => {
    console.log("El valor de 'user' en PromotionalRoulette ha cambiado a:", newValue);
    // Si el nuevo valor es un objeto, mostramos sus propiedades (keys)
    if (newValue && typeof newValue === 'object') {
        console.log("Propiedades (keys) del objeto 'user':", Object.keys(newValue));
    }
}, { 
...
// --- Lógica de la Ruleta (Funciones ÚNICAS) ---
const startCallback = async () => {
  ...
  try {
    const { data: prizeResult } = await apiClient.post('/api/game/spin', {});
    const prizeIndex = prizes.value.findIndex(p => p.id === prizeResult.id);
    if (prizeIndex === -1) {
        throw new Error('El premio recibido no está sincronizado con la ruleta.');
    }
    myLucky.value.play();
    setTimeout(() => { myLucky.value.stop(prizeIndex); }, 100);
  } catch (error) {
    isLoadingPrize.value = false;
    Swal.fire({ 
        title: '¡Oops!', 
        text: error.response?.data?.message || 'Ocurrió un error al intentar girar.', 
        icon: 'error', 
...
const endCallback = (prize) => {
    ...
    if(user.value && user.value.spinsAvailable > 0) {
        user.value.spinsAvailable--;
    }
...
const fetchPrizes = async () => {
  try {
    const { data } = await apiClient.get('/api/admin/prizes');
...
// --- Configuración visual y Ciclo de Vida ---
